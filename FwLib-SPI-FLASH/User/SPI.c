...
void FLASH_SPI_Wait()
{
	...
	FLASH_SPI_SendByte(0x05);	/* 读取状态命令 */
...
void FLASH_SPI_ReadData(uint32_t addr, uint8_t * arr, uint32_t stopReadNum)
{
	...
	FLASH_SPI_SendByte(0x03);	/* 读取命令 */
	
	
	FLASH_SPI_SendByte( (uint8_t)(addr >> 16));	/* 发送24位地址 */
	FLASH_SPI_SendByte( (uint8_t)(addr >> 8));
	FLASH_SPI_SendByte( (uint8_t)(addr) );
	while( stopReadNum-- )
	{
		
		*arr++ = FLASH_SPI_ReadByte(0x00);
	}
